	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.bisaixiangmu" placeholder="比赛项目"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('chuangxinnenglibisai','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('chuangxinnenglibisai','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('chuangxinnenglibisai','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('chuangxinnenglibisai','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('chuangxinnenglibisai','项目统计')">项目统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				ref="table"
				v-if="btnAuth('chuangxinnenglibisai','查看')"
				:data="list"
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('chuangxinnenglibisai','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('chuangxinnenglibisai','报名')" type="success" @click="bisaibaomingCrossAddOrUpdateHandler(scope.row,'cross','','','')">报名</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'项目统计'" width="70%">
			<div  id="bisaixiangmuEchart1" style="width:100%;height:600px;"></div>
...
<script setup>
	...
	//基础信息
	const tableName = 'chuangxinnenglibisai'
	const formName = '创新能力比赛'
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.bisaixiangmu&&searchQuery.value.bisaixiangmu!=''){
			params['bisaixiangmu'] = '%' + searchQuery.value.bisaixiangmu + '%'
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var bisaixiangmuEchart1 = echarts.init(document.getElementById("bisaixiangmuEchart1"),'macarons');
			context?.$http({
    ...
    const bisaibaomingCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			bisaibaomingFormModelRef.value.init(row.id,'cross','报名',row,'chuangxinnenglibisai',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
