	...
	<div class="app-contain" :style='{"padding":"0px 0","margin":"20px auto 40px","alignItems":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"86%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">讲师名称</div>
					<div  class="info_text" >{{detail.jiangshimingcheng}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">讲座地点</div>
					<div  class="info_text" >{{detail.jiangzuodidian}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('jiangzuoxinxi','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('jiangzuoxinxi','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('jiangzuoxinxi','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		...
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="讲座详情" name="first">
				<div v-html="detail.jiangzuoxiangqing"></div>
			</el-tab-pane>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
			...
			title.value = res.data.data.jiangzuomingcheng
